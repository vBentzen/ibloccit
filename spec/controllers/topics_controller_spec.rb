...
RSpec.describe TopicsController, type: :controller do
  ...
  describe "POST create" do
    it "increases the number of topic by 1" do
      expect{ post :create, topic: {name: Faker::StarWars.character, description: Faker::Lorem.paragraph}}.to change(Topic,:count).by(1)
    end

    it "assigns the new topic to @topic" do
      post :create, topic: {name: Faker::StarWars.character, description: Faker::Lorem.paragraph}
