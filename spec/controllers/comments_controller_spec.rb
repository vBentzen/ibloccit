...
RSpec.describe CommentsController, type: :controller do
	...
	let(:other_user) { User.create!(name: Faker::Name.name, email: Faker::Internet.email, password: "helloworld", role: :member) }
  let(:my_topic) { Topic.create!(name: Faker::Name.name, description: Faker::Lorem.paragraph) }
  let(:my_post) { my_topic.posts.create!(title: Faker::Name.name, body: Faker::Lorem.paragraph, user: my_user) }
	...
	context "member user doing CRUD on a comment they don't own" do
	 	...
	 	describe "POST create" do
	 		it "increases the number of comments by 1" do
	 			expect{ post :create, post_id: my_post.id, comment: {body: Faker::ChuckNorris.fact} }.to change(Comment,:count).by(1)
	 		end

	 		it "redirects to the post show view" do
	 			post :create, post_id: my_post.id, comment: {body: Faker::ChuckNorris.fact}
